...
SELECT DISTINCT movie.title, COUNT(casting.actorid) FROM movie 
  JOIN casting 
ON movie.id = casting.movieid
  JOIN actor
ON actor.id = casting.actorid
...
-- 15. List all the people who have worked with 'Art Garfunke
SELECT DISTINCT name FROM actor
  JOIN casting
ON actor.id = casting.actorid
  JOIN movie
ON movie.id = casting.movieid
  WHERE movie.title IN (SELECT movie.title FROM movie
JOIN casting 
  ON movie.id = casting.movieid
JOIN actor
  ON actor.id = casting.actorid
 WHERE actor.name = 'Art Garfunkel') AND name NOT LIKE '%Art Garfunkel%';